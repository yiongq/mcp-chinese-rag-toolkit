...
- Five judge tasks — `judgeClaimSupport`, `judgeReverseQuestions`, `judgeContextUsefulness`, `judgeStatementClassification`, `judgeContextAttribution` — each build a prompt and parse the judge's JSON (tolerant of code fences and surrounding prose) into the structured input one scoring metric consumes. Output is validated strictly against the target shape.
